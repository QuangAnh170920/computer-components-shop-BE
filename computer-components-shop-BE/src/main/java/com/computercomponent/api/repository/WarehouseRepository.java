...
public interface WarehouseRepository extends JpaRepository<Warehouse, Long> {
    ...
    @Query(value = "select new com.computercomponent.api.dto.WarehouseManagementDTO(wh.id, wh.code, wh.name, wh.supplier, wh.description, wh.status, p.code, p.name, wh.type, wh.transactionDate, wh.totalQuantity, wh.totalPrice) " +
    ...
    @Query(value = "select new com.computercomponent.api.response.WarehouseDetail(wh.id, wh.code, wh.name, wh.supplier, wh.description, wh.status, p.code, p.name, wh.type, wh.transactionDate, wh.totalQuantity, wh.totalPrice) " +
            "from Warehouse as wh " +
            "left join  Products p on wh.productId = p.id " +
            "where wh.id = :id ",
            countQuery = "select count(wh) from Warehouse as wh " +
